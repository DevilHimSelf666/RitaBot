...
module.exports = function run (data)
{
   ...
   db.getStats(function getStats (stats)
   {
      ...
      const translationGlobalStats =
         ...
         `:chart_with_upwards_trend:  Evil Translate has translated **\`${stats[0].translation}\`**  for these servers\n\n` +
            ...
            async function getServerInfo (server)
            {
               ...
               if (!target)
               {
                  ...
                  const targetServer = `**\`\`\`${serverID} - Server Tranlation Stats\`\`\`**\n` +
                     `Server Joined Evil Translate Network: \`\`\`${server[0].createdAt}\`\`\`\n` +
                     `:bar_chart:  In total **\`${server[0].message}\`** messages in this server have been sent\n\n` +
                     `:chart_with_upwards_trend:  Evil Translate has translated **\`${server[0].translation}\`**  for this server\n\n` +
